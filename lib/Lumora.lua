...
function Library:Create(name)
    ...
    local toggleBtn = create("TextButton", {Size = UDim2.new(0,50,0,50), Position = UDim2.new(0,20,0.5,-25), BackgroundColor3 = COLORS.Accent, Text = "L", Font = Enum.Font.GothamBold, TextColor3 = Color3.new(1,1,1), TextSize = 22, Parent = sg})
    ...
    create("TextLabel", {Size = UDim2.new(1,-40,1,0), Position = UDim2.new(0,14,0,0), BackgroundTransparency = 1, Text = name or "Lumora", Font = Enum.Font.GothamBold, TextColor3 = COLORS.Text, TextSize = 14, TextXAlignment = "Left", Parent = title})
    ...
    function window:Tab(name)
        ...
        function tab:Toggle(text, default, callback)
            ...
            local tStroke = create("UIStroke", {Color = enabled and COLORS.Accent or COLORS.Outline, Thickness = 1.2, Parent = tog})
            ...
            f.InputBegan:Connect(function(i) if i.UserInputType == Enum.UserInputType.MouseButton1 then enabled = not enabled; tween(tog, nil, {BackgroundColor3 = enabled and COLORS.ToggleOn or COLORS.ToggleOff}); tween(dot, nil, {Position = enabled and UDim2.new(1,-15,0.5,-6) or UDim2.new(0,3,0.5,-6)}); tween(tStroke, nil, {Color = enabled and COLORS.Accent or COLORS.Outline}); callback(enabled) end end)
            return tab
        end

        function tab:Button(text, callback)
            local b = create("TextButton", {Size = UDim2.new(0.9,0,0,32), BackgroundColor3 = COLORS.Darker, Text = text, TextColor3 = COLORS.Text, Font = "GothamSemibold", TextSize = 13, Parent = frame})
